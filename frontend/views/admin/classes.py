...
with st.expander("📅 Semester Config", expanded=False):
    ...
    if success and semesters:
        for s in semesters:
            ...
            c3.write("✅ Aktif" if s["is_active"] else "❌ Nonaktif")
            with c4:
                if st.button("Edit", key=f"sem_edit_{s['id']}"):
                    st.session_state.edit_semester = s
        if st.button("➕ Tambah Semester"):
            st.session_state.show_add_semester = True
    else:
        st.info("Belum ada semester. Buat semester baru.")

    if st.session_state.get("show_add_semester"):
        with st.form("form_semester", border=True):
            nama = st.text_input("Nama Semester")
            mulai = st.date_input("Tanggal Mulai")
            selesai = st.date_input("Tanggal Selesai")
            if st.form_submit_button("Simpan"):
                ok, _ = api_request("POST", "/semester/", json={
                    "nama_semester": nama, "tanggal_mulai": str(mulai), "tanggal_selesai": str(selesai),
                })
                if ok:
                    st.success("Semester dibuat")
                    st.session_state.show_add_semester = False
                    st.rerun()
